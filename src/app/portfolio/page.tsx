  ...
  const projects = [
    {
      ...
      capabilities: [
        '18 pre-built missions (BUILD, FIX, MVP, DEPLOY, OPTIMIZE, and more)',
        ...
        '31-guide Field Manual (professional-grade architecture templates)',
        'Native MCP tool-search (agents find and load any connected MCP on demand)',
        'Read-only quality gates and measured keep-or-revert loops (v6.2): agents cannot edit the gate files that judge them'
      ],
      perfectFor: [
      ...
      <section className="section-padding bg-gradient-to-br from-gray-50 to-white">
        <div className="container max-w-5xl text-center">
          ...
          <p className="text-sm text-gray-500">
            Updated{' '}
            <time dateTime={PAGE_UPDATED.portfolio}>{formatUpdated(PAGE_UPDATED.portfolio)}</time>
          </p>
        </div>
      </section>
            ...
            <div className="space-y-3">
              {ecosystemPrinciples.map((principle, index) => (
                <div key={index} className="flex items-start gap-3 text-gray-700">
                  <svg aria-hidden="true" className="w-5 h-5 text-green-600 mt-1 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                    ...
                    <ul className="space-y-2">
                      {project.capabilities.map((capability, idx) => (
                        <li key={idx} className="flex items-start gap-2 text-gray-600">
                          <svg aria-hidden="true" className="w-5 h-5 text-primary-600 mt-0.5 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                            <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" />
                          </svg>
